...
Router.post("/deposit/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const total = total_balance + Number(deposit_amount);
...
Router.post("/debitAndCredit/:id", authMiddleware, async (req, res) => {
  // async function debitAndCredit(customerId, amount) {
  const client = await getClient();
  try {
    await client.query("begin");

    //Debit the customer's account
    const { transaction_date, account_no, amount } = req.body;
    const account_id = req.params.id;
    const withdraw_amount = amount;
    const deposit_amount = withdraw_amount;
    ...
    const total_balance = +result.rows[0].total_balance;
    const total = total_balance - amount;
    ...
    if (withdraw_amount <= total_balance) {
      ...
      //     // Credit account B with the same amount

      const result = await client.query(
        "select total_balance from account where account_no=$1",
        [account_no]
      );

      const total_balance = +result.rows[0].total_balance;

      const newTotal = total_balance + Number(amount);

      await client.query(
        "insert into transactions(transaction_date, deposit_amount, account_id, balance) values($1,$2,$3,$4) returning *",
        [transaction_date, deposit_amount, account_id, newTotal]
      );
      await client.query(
        "update account set total_balance = total_balance + $1 where account_no=$2",
    ...
    await client.query("commit");
    res.status(400).send({
      add_success: "Transfer was a success! ",
    });
  } catch (error) {
    await client.query("rollback");
    res.status(400).send({
      add_error: "Error while transfering amount..Try again later.",
    });
  } finally {
    client.release();
  }
